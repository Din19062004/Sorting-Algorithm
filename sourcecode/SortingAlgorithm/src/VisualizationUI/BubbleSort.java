	        ...
	        if (i[0] < myArray.getNbElements() - 1) {
	            if (!comparing[0]) {
	                ...
	                if (elements[j[0]].getValue() > elements[j[0] + 1].getValue()) {
	                    double temp1 = elements[j[0]].getValue();
	                    double temp2 = elements[j[0] + 1].getValue();
	                    elements[j[0]].setValueText(temp2);
	                    elements[j[0] + 1].setValueText(temp1);
	                    
	                    String[] info = {"Cause array[" + j[0] + "] = " + temp1 + " > array[" + (j[0] + 1) + "] = " + temp2 + ", swap them."};
